...
class FrozenLakeFlexibleAgentsEnvCol(gym.Env):
    """
    ...
    def __init__(self, num_agents=2, grid_size=(5, 5), slip_prob=0.2, hole_prob=0.2, seed=None, 
        ...
        super(FrozenLakeFlexibleAgentsEnvCol, self).__init__()
        ...
        for i in range(self.num_agents):
            if not self.agent_done[i]:
                if new_positions[i] in self.holes:
                    rewards[i] = -1 # HELP CONVERGES MAYBE
    ...
    def render_pygame(self, screen_size=400, save_path=None):
        ...
        for i in range(self.grid_size[0]):
            for j in range(self.grid_size[1]):
                ...
                if pos == self.goal_pos:
                    ...
                    text = self.font.render("F", True, (0, 0, 0))
        ...
        # Agent colors - use different colors for each agent
        agent_colors = [
            (255, 0, 0),   # Red for agent 1
            (0, 0, 255),   # Blue for agent 2
            (255, 165, 0), # Orange for agent 3
            (0, 128, 0),   # Green for agent 4
            (148, 0, 211), # Purple for agent 5+
        ]

        # Draw agents with collision detection
        for pos, agent_indices in position_counts.items():
            ...
            if len(agent_indices) > 1:
                ...
                color = agent_colors[min(agent_idx, len(agent_colors)-1)]
                pygame.draw.circle(self.screen, color, rect.center, self.cell_size // 3)
                # Add agent number
                text = self.font.render(f"A{agent_idx+1}", True, (255, 255, 255))
                text_rect = text.get_rect(center=rect.center)
                self.screen.blit(text, text_rect)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                # Instead of quitting pygame, just hide the window
                pygame.display.iconify()
                # Or you can do nothing and let the caller handle quitting
                pass
